...
bool makeMove(Stone board[][MAX_SIZE], int size, string playerMove, bool isFirstPlayerTurn){ //quite safe
    ...
    if (playerMove[playerMove.length()-1] < 'a' || playerMove[playerMove.length()-1] > 'z') return false ; //1a^
    ...
    switch (board[MAX_SIZE-row][col1 - 'a']) {
        case NA:
            board[MAX_SIZE-row][col1 - 'a'] = isFirstPlayerTurn ? X : O;
            history = history + " " + playerMove  ;
            break;
        case X:
            return false;
            break;
...
bool makeRemove(Stone board[][MAX_SIZE], int size, string playerMove, bool isFirstPlayerTurn){ //ff
    if (playerMove == "") return false;
    if (playerMove[0] < '0' || playerMove[0] > '9') return false;
...
void Move(Stone board[][MAX_SIZE], int size, string playerMove, bool isFirstPlayerTurn, int mode){
    if (mode == 1) makeMove(board, MAX_SIZE, playerMove, isFirstPlayerTurn);
    else makeRemove(board, MAX_SIZE, playerMove, isFirstPlayerTurn);
}
...
bool hasWon(Stone board[][MAX_SIZE], int size, bool isFirstPlayerTurn) { //quite safe
    ...
    string cor = "";
    while (history[n] != ' ') cor = history[n--] + cor;
...
void displayHistory(string history, int numOfMoves) { //quite safe
    ...
    while (cin>>c){
        if (c == 's') break;
            ...
            Move(game, MAX_SIZE, cor[n], isFirstPlayerTurn,1);
            ...
            n--;
            Move(game, MAX_SIZE, cor[n], isFirstPlayerTurn,2);
            isFirstPlayerTurn = !isFirstPlayerTurn;
            displayBoardSimple(game, MAX_SIZE);
            cin.ignore();
            cout<<inputCommand;
